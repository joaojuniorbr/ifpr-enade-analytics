# BUSINESS INTELLIGENCE — PROJETO INTEGRADOR

## MINERAÇÃO DE DADOS — LACUNAS DE CONHECIMENTO

### ENADE Analytics — Marco 3

| **Entrega**               | Até o final da Semana 13 (Marco 3)                                          |
| ------------------------- | --------------------------------------------------------------------------- |
| **Equipe**                | João Junior, Alessandro Sondey, Leandro Zeni                                      |
| **Responsável principal** | Leandro Zeni                                                                |
| **Base de referência**    | [Marco 2](./marco-2.md) · [Marco 1](./marco-1.md) · Briefing da coordenação |

_Este documento apresenta a análise de mineração de dados que identifica padrões
e lacunas de conhecimento por eixo temático. Requisitos relacionados: RF04,
RF05._

---

> **Status:** Pendente — preencher após conclusão do Marco 2 e aplicação do
> simulado-piloto.

## 1. Objetivo da análise

<!-- O que se busca responder com a mineração de dados -->

## 2. Dataset utilizado

<!-- Descrever volume, período e variáveis do dataset anonimizado -->

## 3. Técnicas aplicadas

<!-- Ex.: clustering, análise de frequência, correlação por eixo temático -->

## 4. Resultados por eixo temático

| **Eixo**    | **Desempenho médio** | **Lacuna identificada** | **Observações** |
| ----------- | -------------------- | ----------------------- | --------------- |
| _a definir_ |                      |                         |                 |

## 5. Distribuição das lacunas (concentradas vs. distribuídas)

<!-- Responder à pergunta 3 do briefing -->

## 6. Evolução ao longo do semestre

<!-- Responder à pergunta 4 do briefing -->

## 7. Conclusões e recomendações para a coordenação

## 8. Aprovação

| **Equipe do projeto**                  | **Professora orientadora** |
| -------------------------------------- | -------------------------- |
| João Junior, Alessandro Sondey, Leandro Zeni | Lauriana Paludo            |
